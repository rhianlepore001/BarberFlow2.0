...
const SidebarNavItem: React.FC<SidebarNavItemProps> = ({ item, isActive, onClick, theme }) => (
    <button
        ...
        className={`flex items-center w-full text-left gap-3 px-4 py-3 rounded-lg transition-colors duration-200 relative ${
            isActive ? `${theme.bgPrimary} text-background` : 'text-text-secondary hover:bg-card hover:text-text-primary'
        }`}
...
const Sidebar: React.FC<SidebarProps> = ({ user, onLogout, openModal, items, activeView, setActiveView }) => {
    ...
    return (
        <aside className="hidden md:flex flex-col w-64 bg-background border-r border-card fixed h-full p-4">
            ...
            <div className="mt-auto border-t border-card pt-4">
                 <div className="group relative">
                    <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-card transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-text-secondary">{shopLabels.defaultTeamMemberRole}</p> {/* Usando rótulo dinâmico */}
                        </div>
                    ...
                    <div className="absolute bottom-full left-0 mb-2 w-full p-1 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto">
                        <div className="bg-card rounded-lg shadow-lg border border-white/10 p-1">
                             <button 
