...
const EditWorkingHoursForm: React.FC<EditWorkingHoursFormProps> = ({ onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSaving(true);
        setError(null);
        const { error: dbError } = await supabase.from('shop_settings').upsert({
            id: 1, // always upserting the single settings row
            open_days: openDays,
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-between items-center bg-background-dark p-1 rounded-full">
                        {ALL_DAYS.map(day => (
                            <button 
                                ...
                                className={`w-10 h-10 flex items-center justify-center font-bold rounded-full transition-colors text-sm ${openDays.includes(day) ? 'bg-primary text-background-dark' : 'text-text-secondary-dark hover:bg-gray-700'}`}
                ...
                <div className="flex gap-3">
                    <div className="w-1/2">
                        ...
                        <input type="time" id="start-time" value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full bg-background-dark border-2 border-gray-700 rounded-lg py-2 px-3 text-white focus:ring-primary focus:border-primary"/>
                    </div>
                    <div className="w-1/2">
                        <label htmlFor="end-time" className="block text-sm font-medium text-text-secondary-dark mb-1">Fim</label>
                        <input type="time" id="end-time" value={endTime} onChange={e => setEndTime(e.target.value)} className="w-full bg-background-dark border-2 border-gray-700 rounded-lg py-2 px-3 text-white focus:ring-primary focus:border-primary"/>
                    </div>
                </div>
